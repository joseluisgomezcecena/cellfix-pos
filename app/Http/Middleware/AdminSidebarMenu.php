...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            //App Config dropdown — SOLO admins con business_settings.access o celfix.app_config.access.
            //Se muestra al FINAL del sidebar (order 99) — es configuración de datos que consume
            //la app Flutter (sucursales, promos, beneficios).
            if (auth()->user()->can('business_settings.access')
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        $sub->url(
                            action([\App\Http\Controllers\AppConfig\LocationController::class, 'index']),
                            'Sucursales (App)',
                            ['icon' => '', 'active' => request()->segment(1) == 'app-config' && request()->segment(2) == 'locations']
                        );
                        $sub->url(
                            action([\App\Http\Controllers\AppConfig\PromoController::class, 'index']),
                            'Promos',
                            ['icon' => '', 'active' => request()->segment(1) == 'app-config' && request()->segment(2) == 'promos']
                        );
                        $sub->url(
                            action([\App\Http\Controllers\AppConfig\BenefitController::class, 'index']),
                            'Beneficios',
                            ['icon' => '', 'active' => request()->segment(1) == 'app-config' && request()->segment(2) == 'benefits']
                        );
                    },
                    ['icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
