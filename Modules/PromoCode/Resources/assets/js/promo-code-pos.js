...
(function($) {
    ...
    var PromoCodePOS = {
        ...
        bindEvents: function() {
            ...
            // Handle promo type radio button changes
            $('input[name="promo_type"]').on('change', function() {
                self.togglePromoTypeSection();
            });

            // Initialize section visibility based on default selection
            this.togglePromoTypeSection();
        },

        togglePromoTypeSection: function() {
            var promoType = $('input[name="promo_type"]:checked').val();
            console.log('[PromoCode] Promo type changed to:', promoType);

            if (promoType === 'cellphone') {
                // Hide company section, show cellphone info
                $('#promo_company_section').hide();
                $('#promo_cellphone_section').show();
            } else {
                // Show company section, hide cellphone info
                $('#promo_company_section').show();
                $('#promo_cellphone_section').hide();
            }
        ...
        applyPromoCode: function() {
            var self = this;
            var promoType = $('input[name="promo_type"]:checked').val() || 'company';
            console.log('[PromoCode] applyPromoCode called with promo type:', promoType);
            ...
            // Determine which endpoint to call based on promo type
            if (promoType === 'cellphone') {
                this.applyCellphonePromo(cartItems);
            } else {
                this.applyCompanyPromo(cartItems);
            }
        },

        applyCompanyPromo: function(cartItems) {
            var self = this;
            var companyName = $('#promo_company_name').val();
            ...
            console.log('[PromoCode] Making AJAX request to /promo-codes/calculate for company:', companyName);
        ...
        applyCellphonePromo: function(cartItems) {
            var self = this;
            console.log('[PromoCode] Making AJAX request to /promo-codes/calculate-cellphone');

            $.ajax({
                url: '/promo-codes/calculate-cellphone',
                method: 'POST',
                data: {
                    cart_items: cartItems,
                    _token: $('meta[name="csrf-token"]').attr('content')
                ...
                success: function(response) {
                    console.log('[PromoCode] Cellphone promo AJAX success response:', response);
                    if (response.success) {
                        self.handlePromoCodeSuccess(response);
                    } else {
                        self.showError(response.message || 'Error al aplicar promoción de celulares');
                    }
                },
                error: function(xhr) {
                    console.error('[PromoCode] Cellphone promo AJAX error:', xhr);
                    var message = 'Error al aplicar promoción de celulares';
                    if (xhr.responseJSON && xhr.responseJSON.message) {
                        message = xhr.responseJSON.message;
                    }
                    self.showError(message);
        ...
        getCartItems: function() {
            ...
            $('table#pos_table tbody tr.product_row').each(function() {
                ...
                items.push({
                    ...
                    category: category,
                    row_index: rowIndex,
                    is_cellphone: $row.attr('data-is_cellphone') || $row.data('is_cellphone') || '0'
                });
            });
