                    ...
                    <div class="col-sm-12">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control mousetrap" id="search_product_for_layaway" placeholder="@lang('layaway::lang.search_product_placeholder')"
                                   disabled="{{empty($default_location) ? 'disabled' : ''}}"
                                   autofocus="{{!empty($default_location) ? 'autofocus' : ''}}" />
                        </div>
...
$(document).ready(function(){
    ...
    // Focus on search input if location already selected
    if ($('#business_location_id').val()) {
        $('#search_product_for_layaway').focus();
    }
});
</script>
@endsection