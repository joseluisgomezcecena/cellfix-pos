...
<tr class="product_row @if(!empty($is_repair_brand)) repair-row @endif" data-row_index="{{$row_count}}" data-category="{{$product->category_name ?? ''}}" data-brand="{{$product->brand ?? ''}}" data-is_repair="{{ !empty($is_repair_brand) ? 1 : 0 }}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	<td>
		...
		<br>
		<span class="promo-discount-indicator hide" style="display:none; color: green; font-weight: bold; font-size: 12px;">
			<i class="fa fa-tag"></i> <span class="promo-discount-text"></span>
		</span>
		...
		@php
			...
			$is_affiliate_discount = false;

			if(!empty($discount)) {
				$discount_type = $discount->discount_type;
				$discount_amount = $discount->discount_amount;

				// Check if this is an affiliate discount (priority = 10)
				if(isset($discount->priority) && $discount->priority == 10) {
					$is_affiliate_discount = true;
				}
			}
